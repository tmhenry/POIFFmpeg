...
C<strftime> (default for compatibity reasons but deprecated) or
C<normal>. See the drawtext_expansion, Text expansion section
below for details.
...
If B<expansion> is set to C<strftime> (which is the default for
...
If B<expansion> is set to C<normal> (which will be the default),
...
This filter accepts a single option B<mode> specifying the mode,
which can be specified either by specyfing C<mode=VALUE> either
